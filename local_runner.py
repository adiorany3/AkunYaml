...
DEFAULT_ENV = {
    ...
    "AI_ADBLOCK_MODEL": "cx/gpt-6-astra",
...
def apply_responsiveness(path: Path) -> bool:
    ...
    if isinstance(groups, list):
        ...
        compact = {
            "GLOBAL": (["FALLBACK", "AUTO-FAST"] if is_android else ["LOAD-BALANCE", "FALLBACK", "AUTO-FAST"]),
...
def apply_security(path: Path, profile: str, workdir: Path, interval: int, dns_mode: str) -> bool:
    ...
    if is_android:
        # Finalization rewrites DIRECT to GLOBAL. Match rule targets so repeated
        # optimization replaces, rather than duplicates, existing LAN guards.
        lan_targets = {
            tuple(part.strip().lower() for part in rule.split(",")[:2])
            for rule in LAN_DIRECT_RULES
        }
        cleaned = [
            rule for rule in cleaned
            if tuple(part.strip().lower() for part in rule.split(",")[:2]) not in lan_targets
        ]
    ...
    cleaned = [
        rule for rule in cleaned
        if tuple(part.strip().lower() for part in rule.split(",")[:2]) not in compatibility_targets
    ]
    ...
    if is_android and android_banking_enabled():
        ...
        managed_banking_targets = {
            tuple(part.strip().lower() for part in rule.split(",")[:2])
            for rule in banking_rules
        }
        cleaned = [
            rule for rule in cleaned
            if tuple(part.strip().lower() for part in rule.split(",")[:2]) not in managed_banking_targets
        ]
    ...
    if is_android and android_marketplace_live_enabled():
        ...
        marketplace_rules = [
            rule for rule in dict.fromkeys(android_marketplace_live_guard_rules(android_marketplace_live_policy()))
            if tuple(part.strip().lower() for part in rule.split(",")[:2]) not in managed_banking_targets
        ]
        managed_marketplace_targets = {
            tuple(part.strip().lower() for part in rule.split(",")[:2])
            for rule in marketplace_rules
        }
        cleaned = [
            rule for rule in cleaned
            if tuple(part.strip().lower() for part in rule.split(",")[:2]) not in managed_marketplace_targets
        ]
        ...
        if isinstance(groups, list) and "YOUTUBE" in valid_policies:
            for group in groups:
                ...
                desired = {
                    "type": "fallback",
                    "proxies": [
                        name for name in ("WARM-UP-CF", "STREAMING-FAST", "AUTO-FAST", "FALLBACK")
                        if name in group_names
                    ],
...
def main() -> int:
    ...
    args.min_nodes = args.min_nodes if args.min_nodes is not None else _config_int("MIN_OUTPUT_NODES", 6)
