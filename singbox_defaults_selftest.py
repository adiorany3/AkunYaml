...
def check(nodes, expected):
    ...
    primary = [n.name for n in nodes if n.tier != "MANUAL"]
    assert ("automatic" in groups) == bool(primary), groups
    if primary:
        assert groups["automatic"]["outbounds"] == primary
    ...
    for tag in ("proxy", "BANK", "SOCIAL", "VMESS-VIDEO"):
        ...
        assert {n.name for n in nodes} <= set(group["outbounds"]), group
...
def main():
    ...
    print("PASS: manual-only and mixed groups, nonempty resolved references, real sing-box check")
